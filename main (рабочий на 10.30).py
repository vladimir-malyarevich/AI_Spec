    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith('.txt'):
            ...
            with open(file_path, 'rb') as pdf:
                bot.send_photo(message.from_user.id,pdf, caption='Учебный файл')
...
def handle_buttons(message): #в эту фун-ю добавляем обработку текста через elif
    if message.text == 'Расписание':
       ...
       bot.register_next_step_handler(msg, math_game)
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        markup.add('Меню')
        availible_lessons = list(lessons.keys())[0:level+1] # при таком обрезании списка - [0, 2)
        for lesson in availible_lessons:
